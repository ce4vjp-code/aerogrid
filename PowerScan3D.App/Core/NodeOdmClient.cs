...
public class NodeOdmClient
{
    ...
    public async Task<string> CreateTaskAsync(string name, List<string> photoPaths, string preset)
    {
        ...
        if (preset == "fast")
        {
            options = new { name = name, dsm = true, fast_orthophoto = true, orthophoto_resolution = 10.0 };
        }
        else if (preset == "highres")
        {
            options = new { name = name, dsm = true, orthophoto_resolution = 2.0, feature_quality = "ultra" };
        }
        else // corridor / default
        {
            options = new { name = name, dsm = true, orthophoto_resolution = 5.0, feature_quality = "medium" };
        }
    ...
    public async Task<NodeOdmTaskInfo> GetTaskInfoAsync(string uuid)
    {
        ...
        var json = await response.Content.ReadAsStringAsync();
        return JsonSerializer.Deserialize<NodeOdmTaskInfo>(json) ?? new NodeOdmTaskInfo();
    }
